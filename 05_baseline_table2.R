################################################
# baseline table for sld analysis
###############################################
...
masld <- read.csv("clean_data/nhanes_masld.csv")
...
masld_table1 <- masld_table1 %>%
  filter(!is.na(LIVER_GROUP2))
...
table(masld$LIVER_GROUP2)

baseline_table <- baseline_table %>%
  modify_header(
    stat_1 ~ "**No hepatic steatosis**<br>N = 9,159",
    stat_2 ~ "**Metabolic dysfunction–associated steatotic liver disease**<br>N = 2,060",
    stat_3 ~ "**Metabolic dysfunction–associated steatohepatitis**<br>N = 416",
    p.value ~ "**p-value**"
  )
...
baseline_table %>%
   as_flex_table() %>%
   save_as_docx(path = "baseline_characteristics_masld.docx")
